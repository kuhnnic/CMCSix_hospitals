...
let filters = { validity:'all', hospital:'all', specialty:'all', station:'all', gender:'all', feature:'all', room:'', search:'' };
const $ = s => document.querySelector(s);
const E = { sync:$('#sync'), metrics:$('#metrics'), bars:$('#statusBars'), beds:$('#beds'), listInfo:$('#listInfo'), context:$('#contextLabel'), reset:$('#resetFilters'), validity:$('#validity'), hospital:$('#hospital'), specialty:$('#specialty'), station:$('#station'), gender:$('#gender'), feature:$('#feature'), room:$('#room'), search:$('#search') };
...
function normBed(b){return {...b, room:String(b.room||''), bed:String(b.bed||''), station_id:b.station_id||'', gender:['unassigned','female','male'].includes(b.gender)?b.gender:'unassigned', status:'free'}}
function buildRows(){
  ...
  rows = beds.map(b => {
    ...
    return {...b, hospital_name:hospital.name||'', validity_area:hospital.validity_area||'', hospital_place:hospital.place||'', station_name:station.name||b.station||'', station_code:station.code||'', station_floor:station.floor||'', specialty:station.specialty||b.specialty||''};
  });
}
async function loadData(){
  ...
  try {
    ...
    const [hRes, sRes, bRes] = await Promise.all([
      ...
      sb.from(TABLE).select('*').eq('status','free').order('hospital_id',{ascending:true}).order('room',{ascending:true}).order('bed',{ascending:true})
    ]);
...
function filtered(base = rows){
  ...
  if(filters.search){
    ...
    r = r.filter(x => [x.validity_area,x.hospital_name,x.hospital_place,x.specialty,x.station_name,x.room,x.bed,x.type,x.notes,'Frei',label(GENDER,x.gender)].join(' ').toLowerCase().includes(q));
  }
...
function renderMetrics(data){
  E.metrics.innerHTML = [
    ...
    ['Spitäler',uniq(data.map(r=>r.hospital_id)).length,'sichtbar'],
    ['Stationen',uniq(data.map(r=>r.station_id)).length,'sichtbar'],
    ['Fachgebiete',uniq(data.map(r=>r.specialty)).length,'sichtbar']
  ].map(m=>`<article class="metric"><span>${m[0]}</span><strong>${m[1]}</strong><small>${m[2]}</small></article>`).join('');
}
function renderBars(data){
  const total = rows.length || 1;
  const pct = Math.round((data.length / total) * 100);
...
function renderContext(data){
  ...
  E.context.textContent = parts.join(' · ');
  E.listInfo.textContent = `${data.length} von ${rows.length} freien Betten`;
}
...
function bind(){
  ['validity','hospital','specialty','station','gender','feature'].forEach(key => E[key].onchange = e => { filters[key]=e.target.value; render(); });
  E.room.oninput = e => { filters.room = e.target.value.trim(); render(); };
  E.search.oninput = e => { filters.search = e.target.value.trim(); render(); };
  E.reset.onclick = () => { filters = { validity:'all', hospital:'all', specialty:'all', station:'all', gender:'all', feature:'all', room:'', search:'' }; sortKey='validity_area'; sortDir='asc'; render(); };
}
