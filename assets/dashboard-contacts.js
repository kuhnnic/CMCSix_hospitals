(function(){
  ...
  var WRITE_TABLES=[API_TABLE,'sasis_hospitals'].filter(function(v,i,a){return v&&a.indexOf(v)===i});
  ...
  async function load(){
    ...
    try{
      ...
      console.warn(e);loadError=e&&e.message?e.message:'SASIS-Spitäler konnten nicht geladen werden.';hospitals=[];sync('SASIS-Kontakte konnten nicht geladen werden','err');render();
    }
  }
  ...
  async function saveRow(id,rowEl){var btn=rowEl.querySelector('[data-save]'),state=rowEl.querySelector('[data-state]'),payload={};rowEl.querySelectorAll('[data-field]').forEach(function(i){payload[i.getAttribute('data-field')]=i.value.trim()});btn.disabled=true;state.textContent='speichert ...';for(var i=0;i<WRITE_TABLES.length;i++){try{var res=await sb.from(WRITE_TABLES[i]).update(payload).eq('id',id).select('id').single();if(!res.error){var h=hospitals.find(function(x){return x.id===id});if(h)Object.assign(h,payload);rowEl.classList.remove('dirty');state.textContent='gespeichert';sync('Kontaktfelder gespeichert','ok');return}}catch(e){console.warn(e)}}btn.disabled=false;state.textContent='Fehler';sync('Kontaktfelder konnten nicht gespeichert werden','err')}
