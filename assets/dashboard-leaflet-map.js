(function(){
  ...
  function parseItem(item){
    ...
    var id=item.getAttribute('data-map-hospital')||'';
    var parts=area.split('·').map(clean).filter(Boolean);
    var place=parts.length?parts[parts.length-1]:'';
    return {id:id,name:name,area:area,place:place,count:isFinite(count)?count:0};
  }
  ...
  function renderRealMap(){
    ...
    items.forEach(function(h,i){
      ...
      marker.on('click',function(){map.setView(c,Math.max(map.getZoom(),12))});
    });
